...
int main() {
  ...
  if (mkfifo("ma_pipe", 0600) == -1) {
    printf("tube existant : on va travailler avec le tube existant\n");
  } else {
    printf("tube cree dans le processus ecrivain\n");
  }

  /* Ouverture du tube pour ecriture */
  tub_nomme = open("ma_pipe", O_WRONLY);
  if (tub_nomme == -1) {
    printf("ouverture tube impossible dans le processus ecrivain\n");
    return EXIT_FAILURE;
  }
