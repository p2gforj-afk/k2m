...
pthread_rwlock_t lockAccess = PTHREAD_RWLOCK_INITIALIZER;
...
int main(int argc, char *argv[])
{
    ...
    pthread_rwlock_destroy(&lockAccess);
    return EXIT_SUCCESS;    
} // Fin main
...
void *virement(void *arg)
{
    ...
    pthread_rwlock_wrlock(&lockAccess);
    cpt1 = 200 - 50;
    sleep(3); // Simulation d'un temps de réponse du SI bancaire
    cpt2 = 100 + 50;
    pthread_rwlock_unlock(&lockAccess);
    sleep(3); // Simulation d'un temps de réponse du SI bancaire
...
void *calcul_somme(void *arg)
{
    ...
    pthread_rwlock_rdlock(&lockAccess);
    somme = cpt1 + cpt2;
    pthread_rwlock_unlock(&lockAccess);
