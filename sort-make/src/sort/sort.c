 ...
 **/
#define SORT_FILENAME_MAX_SIZE 1024
...
void sort(const char * i_file, const char * o_file, unsigned long nb_split){
  ...
  for(cpt = 0; cpt < nb_split; ++cpt){
    filenames[cpt] = (char *) malloc(sizeof(char) * SORT_FILENAME_MAX_SIZE);
    nb_print = snprintf(filenames[cpt],
			SORT_FILENAME_MAX_SIZE,
			"/tmp/tmp_split_%d_%lu.txt",getpid(), cpt);
    if(nb_print >= SORT_FILENAME_MAX_SIZE){
      err(1, "Out of buffer (%s:%d)", __FILE__, __LINE__ );
    }

    filenames_sort[cpt] = (char *) malloc(sizeof(char) * SORT_FILENAME_MAX_SIZE);
  ...
  /* 2 - Sort each file */
  sort_sortFiles(nb_split, (const char **) filenames, (const char **) filenames_sort);

  /* 3 - Merge (two by two) */
  sort_combMerge(nb_split, (const char **) filenames_sort, (const char *) o_file);
...
void sort_sortFiles(unsigned long nb_split, const char ** filenames, const char ** filenames_sort){
...
void sort_combMerge(unsigned long nb_split, const char ** filenames_sort, const char * o_file){

  int nb_print = 0;
  unsigned long cpt = 0;

  char previous_name [SORT_FILENAME_MAX_SIZE];
  ...
  char current_name [SORT_FILENAME_MAX_SIZE];
