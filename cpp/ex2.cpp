...
private:
    ...
    std::vector<std::unique_ptr<Member>>;

public:
    void addLivre(const Book &livre)
    {
    ...
    void display()
    {
        for (const auto &livre : livres)
        {
            ...
            std::cout << "Disponible: " << (livre.is_available ? "Oui" : "Non") << std::endl;
            std::cout << std::endl;
        }
    }
    ...
    {
        auto it = std::find_if(livres.begin(), livres.end(), [&search](const Book &livre)
                               { return livre.titre == search; });
